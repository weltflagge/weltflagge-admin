...
function inferInitialStatus(input: CreateOrderInput) {
  if (input.paymentStatus === "Open") {
    return "PAYMENT_OPEN" as const;
  }

  if (input.printFileStatus === "missing" || !input.printFileName.trim()) {
    return "PRINT_FILES_MISSING" as const;
  }

  if (input.printFileStatus === "received") {
    return "PRINT_FILES_REVIEW" as const;
  }

  if (input.printFileStatus === "problem") {
...
export async function createManualOrder(input: CreateOrderInput): Promise<CreateOrderResult> {
  ...
  if (!orderNumber || !customerName || !customerEmail || !itemName) {
  ...
  const printFileName = input.printFileName.trim();
  const printFileStatus = printFileName ? printStatusMap[input.printFileStatus] : "MISSING";
  ...
  await prisma.order.create({
    data: {
      ...
      status: inferInitialStatus(input),
